...
public:
    /* Resource create from file, resource type can be recognized by file type */
    IResource* Get(const String& filePath);

    /* Material object does not create from file, it can not be reused.
     * when we Get Material object, we create a new object and return it.
     * the name of Material object is auto generated, the input name is unused.
     */
    IResource* Get(ResourceType type, const String& name = "");

    /* user who calls Get(xxx) should call Delete(xxx) after using the resource */
