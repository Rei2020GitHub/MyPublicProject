...
import com.sample.hmssample.appauthdemo.model.auth.HuaweiAuth
...
class MainViewModel(
    isHuaweiMobileServicesAvailable: Boolean
    ) : ViewModel() {
    companion object {
    ...
    private val huaweiAuth: BaseAuth = if (isHuaweiMobileServicesAvailable) HuaweiAuth.getInstance() else HuaweiAppAuth.getInstance()
    ...
    fun onActivityResult(context: Context, requestCode: Int, resultCode: Int, data: Intent?) {
        data?.let { data ->
            when (requestCode) {
                ...
                HuaweiAppAuth.REQUEST_CODE, HuaweiAuth.REQUEST_CODE -> {
